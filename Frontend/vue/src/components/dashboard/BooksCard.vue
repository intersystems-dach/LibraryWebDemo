<template>
  ...
  <v-card>
    ...
    <v-layout justify-end>
      <v-btn small  class="ma-2 primary justify-end" elevation="21" @click="createBookDialog=true"> <v-icon light>mdi-plus</v-icon>New Entry</v-btn>
    ...
    >
      ...
      <template v-slot:item.action="{ item }">
        <div class="actions">
          <v-btn class="primary" block small v-if="item.Friend.ID" elevation="21" @click="selectedBookId=item.id;returnBookDialog=true;">
            Return Book
          </v-btn>
          <v-btn class="primary" block small v-else elevation="21" @click="configureLoanBook(item);loanBookDialog=true;">
            Loan Book
          </v-btn>
        </div>
      </template>
      ...
      <v-card dense>
        ...
        <v-card-actions>
          ...
          <v-btn
            color="blue darken-1"
            text
            @click="createBookDialog = false;resetCreateNewBookForm()"
          >
            Close
          </v-btn>
          <v-btn
      ...
      <v-card dense>
        ...
        <v-card-actions>
          ...
          <v-btn
            color="blue darken-1"
            text
            @click="loanBookDialog = false;resetLoanBook()"
          >
            Close
          </v-btn>
          <v-btn
...
export default {
  ...
  methods: {
    async retrieveAllBooks() {
      ...
      await axios
        .get("http://localhost:52773/LibraryApi/getallbook")
        .then((resp) => {
    ...
    async retrieveAllPeople() {
      //axios call to retrieve all books
      await axios
        .get("http://localhost:52773/LibraryApi/getallperson")
        .then((resp) => {
    ...
    async createNewBook() {
      if(this.$refs.createBookForm.validate()) {
        //Write axios to create the book check the data() variables
        //http://localhost:52773/LibraryApi/addbook?title=newbook3&author=jan&genre=2
      const newbook_URL = `http://localhost:52773/LibraryApi/addbook?title=${this.bookTitle}3&author=${this.authorName}&genre=${this.genre}`
      await axios
        .post(newbook_URL)
        .then((resp) => {
    ...
    async returnBook() {
      ...
      const returnbook_url = `http://localhost:52773/LibraryApi/returnbook/${this.selectedBookId}`
        await axios
        .post(returnbook_url)
        .then((resp) => {
          /* this.people = resp.data.status */
    ...
    async loanBook() {
      if(this.$refs.loanBookForm.validate()) {
        ...
        const loan_url = `http://localhost:52773/LibraryApi/loanbook?Bookid=${this.selectedloanBookId}&FriendsName=${this.loanBookFriend}`
        await axios
        .post(loan_url)
        .then((resp) => {
          /* this.people = resp.data.status */
