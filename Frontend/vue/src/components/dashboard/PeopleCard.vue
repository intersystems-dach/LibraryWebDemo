<template>
  ...
  <v-card>
    ...
    <v-layout justify-end>
      <v-btn small  class="ma-2 primary justify-end" elevation="21" @click="createPersonDialog=true"> <v-icon light>mdi-plus</v-icon>New Entry</v-btn>
    ...
    >
      ...
      <template v-slot:item.action="{ item }">
        <div class="actions">
          ...
          <v-btn class="error" block small elevation="21" @click="deletePerson(item);">
            Delete person
          </v-btn>
        </div>
      </template>
      ...
      <v-card dense>
        ...
        <v-card-actions>
          ...
          <v-btn
            color="blue darken-1"
            text
            @click="createPersonDialog = false;resetCreateNewPersonForm()"
          >
            Close
          </v-btn>
          <v-btn
  ...
  methods: {
    ...
    async createNewPerson() {
      if(this.$refs.createPersonForm.validate()) {
        ...
        const url_add_person = `http://localhost:52773/LibraryApi/addfriend`
        await axios
        
        .post(url_add_person, requestBody)
        .then((resp) => {
          //this.items = resp.data.status
          console.log(resp);
        })
    ...
    async deletePerson(item){
      ...
      const url_del_person = `http://localhost:52773/LibraryApi/removeperson?Personid=${person_id}`
